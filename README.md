...
* **Resuming Training:** If training is interrupted or if you want to continue improving a model, you can resume from the latest checkpoint. The training script automatically saves checkpoints (network weights and optimizer state) in the `checkpoints/` folder of the run. To resume, simply use the
same output folder and config, and set the `resume` flag in the config or command (if implemented) so that the script will load the latest checkpoint instead of starting from scratch. (Under the hood, `checkpoint_utils.py` handles saving and loading of models.)
...
One distinctive feature of this codebase is the support for different action distribution types for continuous action spaces. Continuous control tasks (like MuJoCo locomotion or robotics environments) typically require the agent to output real-valued actions within a certain range $[a_{\text{low}}, a_{\text{high}}]$. Two approaches are implemented here:
