...
def plot_learning_curves(base_log_dir: Path, output_dir: Path) -> None:
    ...
    # Group runs by (environment, algorithm)
    for run_dir in base_log_dir.iterdir():
    ...
    for cfg_path in config_files:
        try:
            ...
            if not all(key in config for key in required_keys):
                ...
                continue
            # --- Add base log dir to config for agent ---
            config['base_log_dir'] = str(base_log_dir)
